...
func InitLoggerByConfig(config *config.LogConfig) (*logging.Logger, error) {
	...
	sql_log_fp, err := logging.NewFileLogWriter(config.LogDir+"/"+config.LogFile+".mysql", false, 500*1024*1024)
	if err != nil {
	...
	info_log_fp, err := logging.NewFileLogWriter(config.LogDir+"/"+config.LogFile, true, 500*1024*1024)
	if err != nil {
		fmt.Println("open file[%s] failed[%s]", config.LogFile, err)
		return nil, err
	}

	err_log_fp, err := logging.NewFileLogWriter(config.LogDir+"/"+config.LogFile+".wf", false, 500*1024*1024)
	if err != nil {
