...
uint16_t rte_rx_callback(uint16_t port_id, uint16_t queue, struct rte_mbuf *pkts[], uint16_t nb_pkts, uint16_t max_pkts, void *user_param)
{
...
struct PortCard* get_match_networkcard(int mac_count, const char **mac_addr)
{
	...
	uint8_t ports = rte_eth_dev_count_avail(); //igb网卡数
