package main

import (
	"fmt"
	"log"
	"net"

	"github.com/ccovers/opgo/protocol/grpc/common"
	"github.com/ccovers/opgo/protocol/grpc/common/pbProto/pb_user"
	"github.com/golang/protobuf/proto"
	"google.golang.org/grpc"
	"google.golang.org/grpc/reflection"
)

func main() {
	lis, err := net.Listen("tcp", ":8080") //监听所有网卡8028端口的TCP连接
	if err != nil {
		log.Fatalf("监听失败: %v", err)
		return
	}

	//创建gRPC服务
	s := grpc.NewServer(grpc.UnaryInterceptor(common.GrpcFunc))

	/**注册接口服务
	 * 以定义proto时的service为单位注册，服务中可以有多个方法
	 * (proto编译时会为每个service生成Register***Server方法)
	 * 包.注册服务方法(gRpc服务实例，包含接口方法的结构体[指针])
	 */
	ser := common.NewServer()
	ser.RegMsgProc(pb_user.EnCmdID_CS_UserInfo, UserInfo)
	pb_user.RegisterServerServer(s, ser)

	/**如果有可以注册多个接口服务,结构体要实现对应的接口方法
	 * user.RegisterLoginServer(s, &server{})
	 * minMovie.RegisterFbiServer(s, &server{})
	 */
	// 在gRPC服务器上注册反射服务
	reflection.Register(s)
	// 将监听交给gRPC服务处理
	err = s.Serve(lis)
	if err != nil {
		log.Fatalf("failed to serve: %v", err)
	}
}

func UserInfo(data []byte) ([]byte, error) {
	userReq := pb_user.CS_UserInfo_Req{}
	err := proto.Unmarshal(data, &userReq)
	if err != nil {
		log.Fatalf("unmashal err: %+v", err)
		return nil, err
	}

	userResp := pb_user.SC_UserInfo_Resp{
		Id:    userReq.Id,
		Name:  fmt.Sprintf("h-%d", userReq.Id),
		Score: 100,
	}
	redata, err := proto.Marshal(&userResp)
	if err != nil {
		log.Fatalf("mashal err: %+v", err)
		return nil, err
	}
	return redata, nil
}
